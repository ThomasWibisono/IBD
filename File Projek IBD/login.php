...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nama_bruder && $password) {
        // koneksi database (pastikan $pdo sudah di-include)
        $stmt = $pdo->prepare("SELECT * FROM login_bruder WHERE nama_bruder = ?");
        $stmt->execute([$nama_bruder]);
        $user = $stmt->fetch();

        if ($user && password_verify($password, $user['password_bruder'])) {
    ...
    <div class="wrapper">
        ...
        <div class="left-panel">
            ...
            <form method="post" action="">
                ...
                <a href="#" class="small-link">Masuk Sebagai Ekonom</a>
            </form>
        </div>

        <!-- RIGHT (logo + halo fic) -->
        <div class="right-section">
            <img src="fic.png" alt="Logo FIC">
        </div>
    </div>
