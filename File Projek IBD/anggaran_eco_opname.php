...
$foto = isset($_SESSION['foto']) ? $_SESSION['foto'] : "foto/thom.jpg"; 
?>
<!DOCTYPE html>
<html lang="id">
<head>
...
<style>
    body{margin:0;font-family:'Segoe UI',Tahoma,sans-serif;background:#f4f4f4;display:flex}
    /* sidebar */
    .sidebar{width:220px;background:#d32f2f;color:white;min-height:100vh;padding:15px 0;}
    ...
    .sidebar a.active{background:yellow;color:black;font-weight:bold;}
    /* main */
    .main{flex:1}
    header{display:flex;align-items:center;justify-content:space-between;background:white;
        padding:10px 20px;box-shadow:0 2px 5px rgba(0,0,0,0.1);}
    .logo{height:60px}
    nav{display:flex;gap:15px}
    nav a{padding:10px 20px;border-radius:25px;background:#1e90ff;color:white;text-decoration:none;font-weight:bold}
    nav a.active{background:orange;color:black}
    .profile-wrapper{position:relative;cursor:pointer}
    .profile-pic{width:50px;height:50px;border-radius:50%;object-fit:cover}
    .dropdown{display:none;position:absolute;right:0;top:60px;background:white;
        box-shadow:0 2px 5px rgba(0,0,0,0.2);border-radius:8px;overflow:hidden}
    .dropdown a{display:block;padding:10px 20px;color:#333;text-decoration:none;font-size:14px}
    ...
    .btn{background:#1e90ff;color:white;padding:8px 18px;border-radius:20px;border:none;cursor:pointer}
    .preview-img{max-width:120px;max-height:70px;border:1px solid #ccc;padding:4px;background:#fafafa}
    .row{display:flex;gap:15px;flex-wrap:wrap;margin-bottom:12px;align-items:center}
    .row label{min-width:140px;font-weight:bold}
    .row input,.row select{padding:6px;border:1px solid #ddd;border-radius:6px}
...
<body>
    ...
    <header>
        ...
        <div class="profile-wrapper" onclick="toggleDropdown()">
            <img src="<?=htmlspecialchars($foto)?>" alt="Profile" class="profile-pic">
